...
def event_to_sparse(
    u: np.ndarray, v: np.ndarray, w: np.ndarray, *, width: int, thresh: float
...
def pack_events(coords_list: Iterable[np.ndarray], feats_list: Iterable[np.ndarray]):
    ...
    lengths = np.fromiter(
        (c.shape[0] for c in coords_list), dtype=np.int64, count=n_events
    )
...
def _infer_hw_from_any(
    raw: np.ndarray, *, fallback_h: int, fallback_w: int
) -> Tuple[int, int]:
    try:
        arr = np.asarray(raw)
    except Exception:
...
def _flatten_plane_batch(
    raw: np.ndarray, *, h: int, w: int, name: str, strict: bool
...
def write_shards_from_root():
    ...
    def render_progress(current: int, total: int, width: int = 40) -> None:
        ...
        if current > 0:
            ...
            eta = "--:--:--"
        print(
            f"\rProcessing events: |{bar}| {current}/{total} ETA {eta}",
            end="",
            flush=True,
        )
    ...
    decomp_context = (
        cf.ThreadPoolExecutor(max_workers=n_decomp) if n_decomp > 0 else nullcontext()
    )
    ...
    with decomp_context as decomp:
        with uproot.open(
            ...
            weights_all = (
                t[cfg.BR_WGT].array(library="np").astype(np.float32).reshape(-1)
            )

            n_events = int(
                min(labels_all.shape[0], weights_all.shape[0], int(t.num_entries))
            )
            if (
                labels_all.shape[0] != n_events
                or weights_all.shape[0] != n_events
                or int(t.num_entries) != n_events
            ):
                warnings.warn(
                    f"entry count mismatch: tree={int(t.num_entries)} {cfg.BR_Y}={labels_all.shape[0]} {cfg.BR_WGT}={weights_all.shape[0]}; using n_events={n_events}"
                )
            ...
            for start in range(0, n_events, cfg.CHUNK_EVENTS):
                ...
                faulthandler.dump_traceback_later(
                    int(getattr(cfg, "FAULTHANDLER_TIMEOUT", 120)), repeat=False
                )
                try:
                    a = t.arrays(
                ...
                if start == 0:
                    ...
                    if (hu, wu) == (hv, wv) == (hw_, ww_) and (hu, wu) != (h, w):
                        warnings.warn(
                            f"cfg.H/cfg.W={h}x{w} do not match data={hu}x{wu}; using inferred H/W from file"
                        )
                        (h, w) = (int(hu), int(wu))
                        hw = int(h) * int(w)
                ...
                for j in range(stop - start):
                    ...
                    try:
                        ...
                        if u.size != hw:
                            raise ValueError(
                                f"{cfg.BR_U}[{gi}] size {u.size} != {hw} (H*W)"
                            )
                        ...
                        if bad_logged < cfg.MAX_BAD_EVENT_LOG:
                            warnings.warn(
                                f"{cfg.BR_U}[{gi}] malformed ({type(e).__name__}: {e}); zero-filling"
                            )
                            bad_logged += 1
                    ...
                    try:
                        ...
                        if v.size != hw:
                            raise ValueError(
                                f"{cfg.BR_V}[{gi}] size {v.size} != {hw} (H*W)"
                            )
                    except Exception as e:
                        if cfg.STRICT_SHAPES:
                            raise
                        ...
                        if bad_logged < cfg.MAX_BAD_EVENT_LOG:
                            warnings.warn(
                                f"{cfg.BR_V}[{gi}] malformed ({type(e).__name__}: {e}); zero-filling"
                            )
                            bad_logged += 1
                    ...
                    try:
                        ...
                        if wplane.size != hw:
                            raise ValueError(
                                f"{cfg.BR_W}[{gi}] size {wplane.size} != {hw} (H*W)"
                            )
                    except Exception as e:
                        if cfg.STRICT_SHAPES:
                            raise
                        ...
                        if bad_logged < cfg.MAX_BAD_EVENT_LOG:
                            warnings.warn(
                                f"{cfg.BR_W}[{gi}] malformed ({type(e).__name__}: {e}); zero-filling"
                            )
                            bad_logged += 1
                    ...
                    if bad_evt:
                        ...
                        try:
                            (c, fe, nnz_true) = event_to_sparse(
                                u, v, wplane, width=w, thresh=thresh
                            )
                        except Exception as e:
                            if cfg.STRICT_SHAPES:
                                raise
                    ...
                    if n_acc == cfg.SHARD_EVENTS:
                        (coords_t, feats_t, starts_t) = pack_events(
                            coords_acc, feats_acc
                        )
                        _atomic_torch_save(
                            {
    ...
    if bad_events:
        print(
            f"warning: {len(bad_events)} events had missing/malformed data and were zero-filled (see index.pt: bad_events)"
        )
