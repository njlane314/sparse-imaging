...
class ShardDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, shards_dir: str, event_indices: np.ndarray, cache_size: int = 2):
        ...
        self.weights = self.weights_all[self.event_indices].astype(
            np.float32, copy=False
        )
        self.shard_ids = (self.event_indices // self.shard_events).astype(
            np.int64, copy=False
        )
        self.local_ids = (
            self.event_indices - self.shard_ids * self.shard_events
    ...
    def _slice_one(d, local: int):
        ...
        feats = d["feats"][s:e].to(dtype=torch.float32)
        return (coords, feats)
