...
class MultiPlaneSparseEvidenceUResNet(nn.Module):
    """
    Shared 2D sparse UResNet over LArTPC planes with explicit evidence accumulation.

    For each plane p:
        Sparse image x_p
          -> shared UResNet decoder features h_p(u)
          -> 1x1 sparse conv to local evidence map a_p(u)  [one scalar per active site]
          -> global sum pooling to plane evidence e_p = sum_u a_p(u)

    Final event logit:
        s = sum_p e_p                      # strict additive evidence model
        s = sum_p e_p + delta(z_bar)       # optional small non-additive correction

    Notes
    -----
    - `event_logit` is the raw scalar to feed into BCEWithLogitsLoss.
    - `return_parts=True, return_maps=True` returns the per-plane local evidence maps.
    - If you want the cleanest LLR-like surrogate, use `use_correction=False`.
    - If shared local readout is too rigid for induction vs collection morphology,
      set `separate_plane_heads=True`.
    """
    ...
    def _infer_batch_column(coords: torch.Tensor) -> int:
        """
        MinkowskiEngine >= 0.4 uses batch index in the first column.
        Older versions used the last column.
        Prefer whichever edge column varies; otherwise default to 0.
        """
    ...
    def _global_sparse_to_dense(cls, x: ME.SparseTensor, batch_size: int) -> torch.Tensor:
        """
        Convert a globally pooled sparse tensor into a dense [B, C] tensor,
        inserting zeros for missing batch items.
        """
    ...
    class ResBlock(nn.Module):
        def __init__(self, cin: int, cout: int, dimension: int = 2):
            super().__init__()
            self.conv1 = MultiPlaneSparseEvidenceUResNet._subm_conv(
                cin, cout, kernel_size=3, dimension=dimension, bias=False
            )
            self.n1 = MultiPlaneSparseEvidenceUResNet.SparseLayerNorm(cout)
            self.conv2 = MultiPlaneSparseEvidenceUResNet._subm_conv(
                cout, cout, kernel_size=3, dimension=dimension, bias=False
            )
            ...
            if cin != cout:
                self.proj = nn.Sequential(
                    ...
                    MultiPlaneSparseEvidenceUResNet.SparseLayerNorm(cout),
                )
        ...
        def __init__(self, cin: int, cout: int, dimension: int = 2):
            super().__init__(
                ME.MinkowskiConvolution(
                    cin, cout, kernel_size=2, stride=2, dimension=dimension, bias=False
                ),
                MultiPlaneSparseEvidenceUResNet.SparseLayerNorm(cout),
    ...
    def __init__(
        ...
        *,
        preset: Optional[str] = "base",
        base: Optional[int] = None,
        blocks: Optional[Sequence[int]] = None,
        embed_dim: int = 128,
        dimension: int = 2,
        use_correction: bool = False,
        separate_plane_heads: bool = False,
    ):
        super().__init__()
        ...
        # Shared UResNet trunk.
        self.stem = nn.Sequential(
            self._subm_conv(in_ch, self.base, kernel_size=3, dimension=self.dimension, bias=False),
            self.SparseLayerNorm(self.base),
            ME.MinkowskiReLU(inplace=False),
        )
        ...
        for i, nb in enumerate(self.blocks_cfg):
            self.enc_blocks.append(
                nn.Sequential(*[self.ResBlock(ch, ch, dimension=self.dimension) for _ in range(nb)])
            )
        ...
        self.bottleneck = nn.Sequential(
            self.ResBlock(ch, ch, dimension=self.dimension),
            self.ResBlock(ch, ch, dimension=self.dimension),
        )
        ...
        for i in reversed(range(len(self.blocks_cfg) - 1)):
            ...
            self.dec_blocks.append(
                nn.Sequential(
                    self.ResBlock(skip_ch + skip_ch, skip_ch, dimension=self.dimension),
                    *[
                        self.ResBlock(skip_ch, skip_ch, dimension=self.dimension)
                        for _ in range(max(0, self.blocks_cfg[i] - 1))
                    ],
                )
            )
            ch = skip_ch
        ...
        # Local evidence head(s): one scalar per active site.
        # No bias here; plane bias is added after pooling so it does not scale with occupancy.
        if self.separate_plane_heads:
            self.local_heads = nn.ModuleList(
                [
                    ME.MinkowskiConvolution(
                        self.decoder_ch, 1, kernel_size=1, stride=1, dimension=self.dimension, bias=False
                    )
                    for _ in range(self.num_views)
                ]
            )
        else:
            self.local_head = ME.MinkowskiConvolution(
                self.decoder_ch, 1, kernel_size=1, stride=1, dimension=self.dimension, bias=False
            )

        # Explicit within-plane evidence accumulation.
        self.evidence_sum = ME.MinkowskiGlobalSumPooling()

        # Per-plane embedding for optional cross-plane correction.
        self.embed_pool = ME.MinkowskiGlobalAvgPooling()
        self.embed_proj = nn.Linear(self.decoder_ch, self.embed_dim)

        # Small per-plane calibration after summing local evidence.
    ...
    def _forward_one_plane(
        self,
        x: ME.SparseTensor,
        plane_idx: int,
        batch_size: int,
    ) -> Tuple[torch.Tensor, torch.Tensor, ME.SparseTensor]:
        """
        Returns
        -------
        z : [B, D]
            Per-plane embedding used only for the optional cross-plane correction.
        e : [B, 1]
            Per-plane scalar evidence from summing the local evidence map.
        a : SparseTensor
            Local evidence map on sparse sites, one channel.
        """
        feat = self._uresnet_features(x)

        a = self._local_head_for_plane(plane_idx)(feat)            # local evidence map
        e_sparse = self.evidence_sum(a)                            # pooled plane evidence
        e = self._global_sparse_to_dense(e_sparse, batch_size)     # [B,1]
        e = self.plane_scale[plane_idx] * e + self.plane_bias[plane_idx]

        z_sparse = self.embed_pool(feat)
        z = self._global_sparse_to_dense(z_sparse, batch_size)     # [B,C]
        z = self.embed_proj(z)                                     # [B,D]
    ...
    def forward(
    ...
    ) -> Union[torch.Tensor, Dict[str, Union[torch.Tensor, Dict[str, ME.SparseTensor]]]]:
        ...
        if available_mask is not None:
            ...
            none = ~mask.any(dim=1, keepdim=True)
            if none.any():
                mask = mask.clone()
                mask[none.expand_as(mask)] = True
        else:
        ...
        Z = torch.stack(z_list, dim=1)               # [B, V, D]
        E = torch.stack(e_list, dim=1).squeeze(-1)   # [B, V]

        if mask is not None:
            mf = mask.to(dtype=E.dtype)
            event_logit = (E * mf).sum(dim=1, keepdim=True)
            z_bar = (Z * mf.unsqueeze(-1)).sum(dim=1) / mf.sum(dim=1, keepdim=True).clamp_min(1.0)
        else:
        ...
        if return_parts:
            out: Dict[str, Union[torch.Tensor, Dict[str, ME.SparseTensor]]] = {
                "event_logit": event_logit,  # [B,1]
                "plane_logits": E,           # [B,V]
                "embeddings": Z,             # [B,V,D]
            }
            if return_maps:
