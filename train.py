...
def train_llr():
    ...
    if nnz_all is not None:
        if nnz_all.shape[0] != n:
            raise ValueError(
                f"index.pt nnz has len={nnz_all.shape[0]} but n_events={n}"
            )
        ...
        if idx_all.size == 0:
            raise ValueError(
                "All events have nnz==0 after sparsification. Check THRESH / branch names / shard generation (bad_events) in index.pt."
            )
        ...
        if labs_train.min() == labs_train.max():
            raise ValueError(
                "After filtering nnz>0, the training split contains only one class. Lower THRESH or inspect index.pt (labels/nnz/bad_events)."
            )
    else:
    ...
    batch_sampler = BalancedBatchSampler(
        ds_train, batch_size=cfg.BATCH_SIZE, seed=cfg.SEED
    )
    val_batch_sampler = BalancedBatchSampler(
        ds_val, batch_size=cfg.BATCH_SIZE, seed=cfg.SEED + 999
    )
    ...
    val_num_batches = int(
        getattr(cfg, "VAL_BATCHES", getattr(cfg, "VAL_NUM_BATCHES", 1))
    )
    ...
    if val_every > 0 and val_cache_batches > 0:
        for _ in range(val_cache_batches):
            try:
                ...
                cached_val_batches.append(next(val_it))
        print(
            f"[val] cached {len(cached_val_batches)} val batches in RAM (VAL_CACHE_BATCHES={val_cache_batches})"
        )

    planes = ("u", "v", "w")
    backbone = make_backbone(cfg.BACKBONE, in_ch=2, embed_dim=cfg.EMBED_DIM).to(device)
    model = MultiViewSetClassifier(
        backbone=backbone, embed_dim=cfg.EMBED_DIM, plane_names=planes
    ).to(device)
    opt = torch.optim.SGD(
        model.parameters(),
        lr=cfg.LR0,
    ...
    for step in range(1, cfg.MAX_STEPS + 1):
        ...
        if logits.shape != y.shape:
            raise RuntimeError(
                f"logits shape {tuple(logits.shape)} != y shape {tuple(y.shape)}; this will broadcast in BCEWithLogitsLoss and can produce zero gradients with balanced batches. Check ME batching / coordinates batch index."
            )
            ...
            with torch.no_grad():
                ...
                for coords_by_plane, feats_by_plane, yv, available_mask in batches:
                    ...
                    if vlogits.shape != yv.shape:
                        raise RuntimeError(
                            f"[val] logits shape {tuple(vlogits.shape)} != y shape {tuple(yv.shape)}; check ME batching / coordinates."
                        )
